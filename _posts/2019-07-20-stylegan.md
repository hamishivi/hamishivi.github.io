...
If you reload the page, the faces will also change - each is simply loaded from an api endpoint that generates a random face each time it is called. Again, the results aren't the best - I simply trained this model for around one day on my own desktop, with a small dataset.
...
Like the anime faces, these images will change if you refresh the page. Again, the results aren't the best - I simply trained this model for around one or two days on my own desktop, with a small dataset.
...
Then, you can use the tf-serving docker image to serve this model. I modified the [basic tf-serve docker image](https://github.com/tensorflow/serving/blob/master/tensorflow_serving/tools/docker/Dockerfile) to also store my saved model, and deployed it using Heroku's docker deployment functionality. This is actually enough to have an ML api up and running, which is really cool!!
...
![Architecture Overview](https://hamishivi.github.io/public/tfapi.png)

## Conclusion

This was a fun and cool little project to work on, and I'd really recommend trying to serve your toy models on the internet! If they're small enough, there are free hosting options, and it's really neat to have something 'live' on the internet that you created. Hopefully this is useful to you if you ever want to host your own TF models, or something similar. 😄
