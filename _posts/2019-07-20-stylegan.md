...
I didn't stop there- after having successfully trained these models, I uploaded them into a docker container and served them using [tf-serve](https://www.tensorflow.org/tfx/guide/serving), which makes turning Tensorflow models into REST APIs nice and easy. I then wrapped them behind a basic Python API for generating anime faces and Pokemon images.

## Anime Faces

The original page loaded ten faces from an API endpoint that generated a random face for each request. The results weren't the best - I simply trained this model for around one day on my personal desktop, with a small dataset.

> **Archive note:** The live generator originally embedded here is no longer online. The training and serving code remains available in the [pokemongenerator repository](https://github.com/hamishivi/pokemongenerator).

## GANemon

Like the anime faces, the original page loaded fresh images from the generator with each request. Again, the results weren't the best - I simply trained this model for around one or two days on my personal desktop, with a small dataset.
...
<img src="https://images.ivison.id.au/2019-07-20-stylegan-01-678e788dce01.png" alt="Architecture diagram: a Flask server sends random inputs to TensorFlow Serving models and returns generated anime or Pokémon images to the browser." width="665" height="317" loading="lazy">
